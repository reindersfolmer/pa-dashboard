...
**Dashboardknoppen** — elk prioriteits- en radaritem heeft drie knoppen: "✓ afgerond", "💤 snooze" en "⏸ parkeer". Dat zijn mailto-links naar Davids eigen Gmail met onderwerp `PA afgerond: <item-id> <titel>`, `PA snooze: <item-id> <titel>` of `PA parkeer: <item-id> <titel>` — één tik op zijn telefoon en de mail staat klaar om te verzenden. De voorgevulde body stelt per knop één vraag: bij afgerond "wat heb je gedaan?" (Davids antwoord voedt de leerlus), bij snooze "tot wanneer?" (interpreteer ruim: "maandag", "24 juli", "volgende week" — geen datum genoemd → standaard 3 werkdagen, en noteer die aanname op het item), bij parkeer de reden. Elke run zoekt in Gmail naar deze commandomails (onderwerp begint met "PA "), leest óók de body, verwerkt ze (status bijwerken + leerlus bij afgerond, zie `references/leren.md`), en filtert ze uit de prioriteiten. Zo werkt het dashboard als app, ook zonder chat.
